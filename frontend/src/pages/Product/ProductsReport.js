...
const OrdersReport = ({data}) => {
    ...
    const downloadPDF = () => {
        ...
        doc.autoTable({
          head: [["Product ID", "Product Name", "Brand", "Quantity", "Price", "SKU", "Availability"]],
          body: data.map((item) => {
            return [item.productId, item.productName, item.brand, item.quantity, 'Rs.' + item.price.toFixed(2), item.SKU, item.inStock ? 'In Stock' : 'Out of Stock'];
          }),
        ...
        doc.save("products.pdf"); //Change name accordingly
      };
