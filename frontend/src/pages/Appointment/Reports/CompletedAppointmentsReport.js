...
const CompletedAppointmentsReport = ({data}) => {
    ...
    const downloadPDF = () => {
        ...
        doc.text("Completed Appointments Report", (pageWidth / 2), 60, { align: "center" });
        // Underline the text
        const textWidth = doc.getStringUnitWidth("Completed Appointments Report") * doc.internal.getFontSize() / doc.internal.scaleFactor;
        ...
        doc.autoTable({
          head: [["Appointment ID", "Veterinary Surgeon", "Pet Owner Name", "Pet Name", "Date"]],
          body: data.map((appointment) => {
            return [appointment.appointmentId, appointment.vet, appointment.ownerName, appointment.petName, appointment.date];
          }),
        ...
        doc.save("completed-appointments.pdf"); //Change name accordingly
      };
...
export default CompletedAppointmentsReport;
